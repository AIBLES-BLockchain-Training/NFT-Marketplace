...
describe('Auction', function () {
  async function deployContractLoadfixture() {
    ...
    const permissions: any = await PermissionsFactory.deploy();
    ...
    const AUCTION_ROLE = await permissions.AUCTION_ROLE();
  ...
  function transformAuctionObject(auction) {
    return {
      id: auction[0],
  ...
  async function createAuctionWithERC1155(tokenId: number) {
    const startTime = await time.latest();
    const endTime = startTime + 3600; // 1 hour later
  ...
  describe('Should test with ERC721', function () {
    ...
    describe('Should test function claim token', function () {
      ...
      it('Should revert if not the auction creator', async function () {
        await expect(nftAuction.connect(bidder1).collectAuctionPayout(0)).to.be.revertedWith(
  ...
  describe('Should test with ERC1155', function () {
    describe('Auction function create and cancel with ERC1155', function () {
      it('Should create an auction', async function () {
        const { startTime, endTime, auctionObject } = await createAuctionWithERC1155(1);
        // console.log(auctionObject);
    ...
    describe('Should test get auction', function () {
      beforeEach(async function () {
        ...
        await createAuctionWithERC1155(1);
        await createAuctionWithERC1155(2);
        await createAuctionWithERC1155(3);
      });
